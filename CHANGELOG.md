...
- **大更新 ⑧-3｜全局下载队列服务**：新增 `Services/IDownloadQueueService` / `DownloadQueueService`——任务与页面解耦，**离开页面不中断、重进页面恢复进度**。
  - 七态状态机：等待中 / 准备中 / 下载中 / 安装中 / 已完成 / 失败 / 已取消（`Models/DownloadTaskStatus`）。
  - 串行执行泵 + **失败自动重试**（3 次指数退避 1s/2s/4s）+ 手动重试（进度与尝试次数清零）。
  - 取消任务**保留已下载文件**（真实安装中断后重新安装可续传）。
  - 进度广播：总体进度 / 已下载 / 剩余 / 速度；真实安装（CMLLib 无逐文件回调）以**阶段状态 + 不确定进度**呈现，演示任务完整模拟速度曲线。
  - 非「版本」类条目在资源站接入（v0.3）前为**演示任务**（明确标注，不产生网络请求、不写磁盘）；「版本」类走真实 CMLLib 安装。
- **大更新 ⑧-2 / ⑧-5｜安装确认对话框**：新增 `Views/InstallConfirmDialog`——「点安装」变为「选择 → 确认」。
  - 版本：加载器单选（Vanilla / Fabric / Forge / NeoForge，来自真实加载器清单）+ 实例名（默认 `版本 · 原版`）+ 安装位置；选择非 Vanilla 时明确提示本次先装原版（加载器安装随 v0.3）。
  - 模组：目标版本 + 所需加载器 + **兼容性预判**（主次版本不符显示黄色警告，不阻断）。
  - 整合包：依赖自动安装摘要 + 新实例名；其它类型：简单确认。演示任务在对话框中预先标注。
- **大更新 ⑧-4｜外壳「下载中 N」全局入口 + 下载队列页**：
  - 标题栏新增常驻入口：有任务时显示「下载中 N」+ 进度环，点击进入新二级路由 `download-queue`（`Views/DownloadQueuePage`）。
  - 队列页：整体进度条、逐任务进度（不确定进度支持）、状态徽章、取消 / 重试 / 移除、清空已完成；真实版本安装完成后显示完成引导 **[启动游戏] [查看实例]**。
- **大更新 ⑧-1｜下载中心类型一级入口重构**：进入页面先见**资源类型卡**（Minecraft 默认选中，共 7 类），类型内浏览 + 搜索 + 下载源筛选。
  - 安装状态真实化：「版本」条目按本地版本目录实时判定（已安装 / 安装中 / 未安装三态徽章，随队列事件自动刷新），不再使用 Mock 静态标记。
  - 新增 `DownloadItemViewModel`（条目 + 安装状态投影）、`CategoryCardViewModel`（类型卡）；页内下载队列面板移除，由全局队列与队列页取代（页内保留可点击的「正在下载 N」摘要条）。

### Changed

- 下载中心条目按钮从「下载」改为「安装」，语义与产品心智模型（我想玩什么 → 选择 → 系统准备 → 完成后去玩）对齐。

### Tests

- 新增 23 个单元测试（总计 **68 个全部通过**）：队列状态机全流程、失败自动重试（注入短退避）、取消传播到安装器、手动重试清零、列表管理（活动任务不可移除、清空已完成保留活动任务）、状态标签与活跃性判定。
- `DownloadQueueService` 增加 internal 构造注入重试退避间隔（测试加速）；进度更新事件广播修复（此前演示任务进度不刷新）。
...
- **大更新 ⑤-4｜页面过渡与内容渐入，并让「界面动画」开关真正生效**：此前设置页的「界面动画」开关只写文件、重启后也不影响任何过渡，现在成为全应用的动画总开关。
  ...
  - **内容渐入**：导航服务在每次前进与返回后，为刚载入的页面挂上 `EntranceThemeTransition`（轻微上移淡入，允许列表项交错出现）；由于页面由 Frame 反射创建、每次导航都是新实例，不存在重复挂载问题。
  - 设置页「界面动画」开关改为委托动画服务：切换即刻落盘并生效，状态栏说明当前效果；「恢复默认设置」也会把开关恢复为默认值并广播。
  - 不引入任何第三方动画库，全部使用 WinUI 内置过渡。
